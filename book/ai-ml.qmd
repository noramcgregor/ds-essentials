...
By providing a bit of jargon busting alongside practical examples and links to further reading, guidance and communities of practice this guide aims to help you on your way to better understanding today's AI in a library context.
...
If we want a computer to do something, we need to give it instructions. A typical **algorithm** provides a set of step-by-step instructions, in order of operation, for solving a problem or performing a task. Batch renaming files on a server so that they're all lower-case for instance ("Look in this location, find any capitalised letter, replace it with its equivalent lower case, task complete").
...
Though AI and machine learning are highly interlinked today, early developments in the field did not always make use of machine learning. **GOFAI** ("good old fashioned AI") is a phrase sometimes used to refer to systems that exhibit apparently "intelligent" behaviour but that are based on rules and algorithmic constraints that humans have coded themselves, rather than having a machine learn these rules from data. Much early AI fell within this paradigm, for instance the famous [ELIZA chatbot](https://en.wikipedia.org/wiki/ELIZA) created in 1966 by MIT professor Joseph Weizenbaum used clever rules to turn statements input by users into questions thrown back at them by the system.
...
#### Defining the task: Do we need machine learning for this? 

Let’s say we have digitised thousands of 19th century books from your library collection. One problem with the associated metadata for each book is that the language field is empty for them, a holdover from historical cataloguing processes that library professionals everywhere will be all too familiar with. Not only do we need to be able to clearly state what language these texts are written in, we'd like to go a step further and list _all_ the languages that may be represented in each. For instance a book primarily written in English may feature within it a multitude of languages, possibly even endangered languages and dialects that could be highly useful for researchers. The scale of this task is such that we simply cannot do it manually, we cannot read all the pages let alone identify every language easily. We need a language-identification system, on the level of sentences, a system that can go through the digitised texts and tell us "sentence X is in language Y" for this task. We will need machine learning.

#### Train a specialised model from scratch or use a pre-trained (foundation model) and fine-tune it?

Depending on the task, we could try to create our own specialised machine learning model and train it, or use one which is already trained and use some techniques to optimise the results. And indeed, up until recently, building and training specialised models from scratch was the norm for many traditional classification tasks relevant in the library world, but the pre-trained foundation models of today have become so powerful that they are quickly taking over as the first port of call for machine learning explorations in every domain, including libraries and cultural heritage.

> There has been a major pardigm shift from building specialised models for specific tasks to using a single, large model that can be prompted or fine-tuned. 

By far the most common approach is to make use of an existing **pre-trained model**, also referred to as a **"foundation"** or **"base"** model which has already been trained on lots and lots of data already. At the very least you will want to always first test out how a pre-trained model fairs before embarking on the building of specialised models which can be costly, time consumming, and require special expertise and loads of labelled data. 
...
Foundation models are typically provided by big companies with access to the resources required for building these models. Some examples of foundation models available in one way or another to the public at the time of writing include:
...
- [BERT](https://huggingface.co/docs/transformers/en/model_doc/bert), a family of models outside the generative paradigm that....

The expectation is that foundation models can be used by researchers, programmers, and other downstream users to develop task-or-domain-specific applications.  And while foundation models can cover a variety of different objectives in terms of input and output, they have proved particularly useful in AI applications due to the huge amounts of data and computation required to build one of these models from the ground up.


**Using a pre-trained model + fine-tuning or prompt engineering**

We can visit the websites above for all of the major pre-trained models but it's also worth checking sites like [Hugging Face](https://huggingface.co/models) and [Kaggle](https://www.kaggle.com/models) which are great places to find existing pre-trained machine learning models of all kinds, usually categorised by tasks they are exceptionally capable of handling: 
...
We select an existing pre-trained model based on the task we want to accomplish (in our use case it's language identification so we might like to start with......TODO which model). We show the base model our unlabelled data (in our example, this might be the text from the OCR pages of our digitised texts, delivered to the model in a structure it can read, like plain text) and see how well it does what we want it to do (ie, it identifies the languages featured on each page with a level of accuracy we expect).
The output that the current state of the model offers is compared to the target output. For instance if the sentence is "tengo mi dos huevos", we want the output to be "spanish".  A standard set-up would involve a model which outputs a probability associated with each possible target - in this case target language - such that all possible targets add up to 1 (this is called multi-class classification).  

Depending on how well the model performs there are several ways in which we can work with the pre-trained model to improve its results. 

**[Prompt engineering](https://www.promptingguide.ai/)** does not change the model itself but involves guiding the model to _respond_ more accurately and contextually by crafting well-structured inputs (prompts). Prompt engineering doesn't change the models weights in any way.

**Retrieval Augmented Generation (RAG)** is another method you can use to help guide a pre-trained model to _respond_ more contextually without changing the weights of the model itself. RAG is a method where the LLM is connected to an external knowledge base or document store, allowing it to fetch relevant real-time information and then generate responses based on that retrieved content. Some recent experiments in using RAG in the library world are being explored for tasks like optimising search of web archive files https://lil.law.harvard.edu/blog/2024/02/12/warc-gpt-an-open-source-tool-for-exploring-web-archives-with-ai/, or https://kdl.kcl.ac.uk/blog/ireal-rag/)

**Fine-tuning** is a more resource heavy optimisation process as it involves taking a pre-trained model and adjusting it to better fit new data we give it. Fine-tuning modifies the model itself by training it on our specialised data which actually changes the weights of the model to improve its performance. Fine-tuning requires labelled data and compute resources and has the potential to be time-consumming, but is also a really useful process for domain-specific tasks. In our fictional example it might be sentences where we know what language they are in and we label each sentence accordingly. This is "supervised" machine learning, in that we are categorising and applying specific labels to the data we will train the model with. Other types are:

- Unsupervised - the model is given data that has not been categorised and labeled and asked to put it into groups (find patterns) without guidance
...
Let's say the current state of the model associates a probability of 0.2 with that input being the "spanish" label. We want the probability to be 1.0, so we take the difference between those two of 0.8, and the algorithm uses this "loss" to update the model.  The algorithm handles all of these moves forwards and backwards through the model, running an input, checking how the output compares with the target, and then updating the parameters of the model based on the difference between target and prediction. 

When we're happy with the way the model handles the training data we have provided it, we're done with training and can now use the model to do "inference", which is the term often used for making a prediction "in the wild" so to speak on completely new data.  We use basically the same algorithm to crank new, unseen data through our model, but rather than update the model we're using it to give us the results we're looking for, mainly our list of languages identified in each of our 19th century digitised books, along with a probablity the system associates with each. We can decide how we use these outputs in our cataloguing and what our appetitite for risk is (ie, only languages identified with 80% certainty and above will listed in the catalogue). 

It's still not out of the question that after exploring all avenues with a pre-trained model you may decide you'd like to train a specialised machine learning model. Perhaps we discover that using existing foundation models won't work well enough for our task (perhaps it [does not yet sufficiently identify low-resource languages](https://www.historica.org/blog/ai-powered-preservation-of-endangered-languages) yet for instance and that is a priority for us) and we may decide we want to try training our own model with our own data. This is always an option but it comes at a cost, both in time and computing resources. 
...
In this guide we’ll focus our attention on just these two particular subfields of AI research areas to give you a general sense of how machine learning is practically applied in the library context today:
...
Let’s have a look at some NLP and Computer Vision use cases in a library setting:
...
**Language models** are a type of machine learning model designed to predict the likelihood of a sequence of text, which means that they can be set up to predict the most likely way to continue a conversation. **Large language models (LLMs)**, such as the models behind ChatGPT, are highly complex neural networks that have been exposed to an enormous amount of text from books, articles, websites, and more. They are pre-trained foundation models that perform natural language processing tasks such as generating and classifying text, answering questions, and translating text and are the backbone of NLP today. There are **small language models (SLM)** too which like LLMs have been trained on an enormous quantity of data however they have been optimised using various clever model compression techniques to use far fewer parameters and in turn not as much memory and computing resource. Current SLMs you might hear of today are:
- DistilBERT
- Gemma
- GPT-4o mini
- Granite
...
SLMs may be right for institutions interested in supporting digital sustainability initiatives as a means of saving computing resource and energy where possible.
...
One of the state of the art applications of machine learning seen in cultural heritage at the moment is [Handwritten Text Recognition (HTR) ](https://libereurope.github.io/ds-topic-guides/atr.html). The idea with HTR is to convert digitised handwritten documents into searchable and machine readable text. To achieve this HTR actually uses a combination of Computer Vision (CV) and Natural Language Processing (NLP).
...
These free online tutorials come highly recommended by library colleagues for learning how to get started using machine learning. Some python knowledge is typically required/advisable for these:
...
The following short hands-on activities/exercises below were developed by the Digital Research Team for British Library staff as part of the [Digital Scholarship Training Programme](https://blogs.bl.uk/digital-scholarship/2024/04/recovered-pages-dstp.html). They are useful and fun for trying things out quickly and to get a sense of the technologies without having to install or download or programme anything.
...
There are of course lists out there with resources for learning more about AI & Machine Learning but I have found this particular guide is exceptionally useful in its coverage and topics selected, particularly as they are quite specifically for Librarians: [Librarians & Faculty: How can I use Artificial Intelligence/Machine Learning Tools? - Research Guides at Northwestern University](https://libguides.northwestern.edu/ai-tools-research/librarians)
