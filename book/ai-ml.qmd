...
Before you begin to embark on any kind of machine learning project it's really useful to take a moment to make sure that you actually need the prediction capabilities of ML capabilities. Often times a simple rules based programmed can resolve an issue and it's not always necessary to use ML. 
...
#### Train a specialised model from scratch or use a foundation model and fine-tune it?
...
Most sites which provide models will include vast amounts of detailed instructions for accessing the model, from setting up your environment, structuring your data, running the model, evaluating the results, updating the model and so on. 

But for our purposes here at the most high-level view, the process of using it would begin with selecting an existing model based on the task we want to accomplish (in our use case it's language identification so you might like to start with (GPT?)). We would then show the foundation model our unlabelled data (in our example, this might be the text from the OCR pages of our digitised texts, delivered to the model in a structure it can read, like plain text) and see how well it does what we want it to do (ie, it identifies the languages featured on each page with a level of accuracy we expect).

We would then look to evaluate whether or not the output that the current state of the model offers is compared to the target output. For instance if we know that a sentence is "tengo mi dos huevos", we want the output to be "spanish".  A standard set-up would involve a model which outputs a probability associated with each possible target - in this case target language - such that all possible targets add up to 1 (this is called multi-class classification).  
...
Librarians are still investigating use cases for new Generative AI applications such as this, but for now at least, ChatGPT is certainly useful as a personal writing assistant or tool to help give you ideas for
